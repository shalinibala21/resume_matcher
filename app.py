    ...
    if submit:
        with st.spinner('Wait for it...'):
            ...
            for item in range(len(relavant_docs)):
                ...
                logging.info(type(relavant_docs))
                logging.info(type(relavant_docs[item]))
                tuple_result=relavant_docs[item]
                logging.info(type(tuple_result[0]))


                match_score=tuple_result[1]
                doc=vars(relavant_docs[item][0])
                ...
                #Displaying Filepath
                st.write("**File** : "+meta_data['name'])

                #Introducing Expander feature
                with st.expander('Show me 👀'): 
                    st.info("**Match Score** : "+str(match_score))
